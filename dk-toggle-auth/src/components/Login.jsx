    ...
    return (
        <div className='w-full max-w-md bg-white/80 backdrop-blur-xl shadow-xl rounded-2xl border border-slate-200/50 p-8 sm:p-10'>
            ...
            <form className='space-y-5'>
                ...
                <button className='w-full py-3 px-4 bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold rounded-xl shadow-sm hover:shadow-md'>Sign in</button>
                <p className='text-center text-sm text-slate-600'>
                    Don't have an account? &nbsp;
                    <button onClick={() => {
                        setToggle(prev => !prev)
                    }} className='font-semibold text-slate-900 hover:text-slate-700'>Sign up</button>
                </p>
            </form>
        </div>
    )
